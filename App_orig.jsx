            ...
            <ul className="space-y-4 overflow-y-auto max-h-[500px] pr-2 custom-scrollbar">
              {uploadedFiles.map((f) => (
                <li key={f.id} className="flex items-center gap-5 text-sm bg-white border border-slate-100 shadow-sm p-5 rounded-[1.5rem] group hover:border-blue-200 transition-all hover:shadow-xl hover:-translate-y-1">
                  ...
                  <div className="flex-1 min-w-0"><p className="truncate text-slate-800 font-black tracking-tight" title={String(f.path)}>{String(f.name)}</p><p className="text-[10px] text-slate-500 mt-1 font-bold uppercase tracking-wider">{String(f.size)} โ€ข <span className="font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded-lg ml-1">{f.rows.length.toLocaleString()} RECORDS</span></p></div>
                      ...
                      <div className="w-full flex flex-col h-full relative">
                        ...
                        <div className="w-full flex items-end justify-center gap-[2px] relative" style={{ height: `${posRatio}%` }}>
                          ...
                          <div className="w-1/4 bg-blue-500 rounded-t-sm transition-all group-hover:opacity-80 shadow-[0_0_8px_rgba(59,130,246,0.3)] relative" style={{ height: `${hIna}%` }}>
                             <span className="absolute -top-7 left-1/2 -translate-x-1/2 text-[8px] font-black text-blue-600 opacity-0 group-hover:opacity-100 transition-opacity">{formatRp(m.inacbg, true)}</span>
                          </div>
  ...
  const renderDpjp = () => {
    ...
    return (
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {[
            { title: 'Top 10 DPJP โ€” Volume Kasus', items: top10Kasus, getVal: d => d.count, color: '#3b82f6', unit: ' kasus' },
                  ...
                  <tbody className="divide-y divide-slate-100 bg-white">
                    {drilldownStats && (
                      <tr className="bg-blue-50 border-b-2 border-blue-200">
                        <td colSpan={7} className="px-5 py-3 font-black text-right text-blue-800 tracking-wider text-xs">RATA-RATA DARI {drilldown.data.length.toLocaleString()} KASUS:</td>
                        <td className="px-5 py-3 text-center font-black text-blue-700 bg-blue-100/30">ALOS: {drilldownStats.avgLos.toFixed(1)}</td>
                        ...
                        <td className="px-5 py-3 text-right font-black text-blue-700">{formatRp(drilldownStats.avgIna)}</td>
                        <td className="px-5 py-3 text-right font-black text-emerald-700">{formatRp(drilldownStats.avgIdrg)}</td>
                        <td className={`px-5 py-3 text-right font-black border-r-4 border-blue-300 ${drilldownStats.avgSel > 0 ? 'text-lime-600' : drilldownStats.avgSel < 0 ? 'text-rose-600' : 'text-slate-500'}`}>{drilldownStats.avgSel > 0 ? '+' : ''}{formatRp(drilldownStats.avgSel)}</td>
                        {compKeys.map(c => <td key={`avg-${c.key}`} className="px-4 py-3 text-right font-bold text-blue-800 bg-blue-100/50 border-r border-blue-100">{formatRpEx(drilldownStats.avgComps[c.key].val)}</td>)}
                      </tr>
                    )}
                    {drilldown.data.slice(0, 300).map((row, i) => {
                      ...
                      return (
                        <tr key={`ddr-${i}`} className="hover:bg-slate-50/80 transition-colors">
                          ...
                          <td className={`${tCell} text-right font-bold text-blue-700 bg-blue-50/20`}>{formatRp(ina)}</td>
