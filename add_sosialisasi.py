...
export const generateSosialisasiPPTX = async ({ ksmName, ksmStats, topCases, topUpPotentials, scatterImageBase64, quadrantInsights }) => {
  ...
  slideCover.addText(KSM: , { x: 1, y: 2.8, w: 8, fontSize: 24, bold: true, color: "ccfbf1", align: "center" });
  slideCover.addText(Tanggal: , { x: 1, y: 3.5, w: 8, fontSize: 14, color: "99f6e4", align: "center" });

  // 2. RINGKASAN FINANSIAL KSM
  const slideFin = pptx.addSlide();
  slideFin.addText(Ringkasan Kinerja - KSM , titleProps);
  ...
  slideFin.addText(${ksmStats.selisih >= 0 ? '+' : ''}, { x: 0.6, y: 3.5, w: 3.8, fontSize: 20, color: ksmStats.selisih >= 0 ? "065f46" : "9f1239", bold: true });
  ...
  // 3. SCATTER PLOT & INSIGHTS (using html2canvas image)
  if (scatterImageBase64) {
    const slideScatter = pptx.addSlide();
    slideScatter.addText(Analisis Kuadran KMKB - KSM , titleProps);
    slideScatter.addText("Posisi INA-CBG berdasarkan Volume dan Selisih", subProps);
    
    // Add the image of the scatter plot
    slideScatter.addImage({ data: scatterImageBase64, x: 0.5, y: 1.3, w: 5, h: 4 });

    // Add Insights Text
    slideScatter.addText("Rekomendasi Sosialisasi & Evaluasi:", { x: 5.8, y: 1.3, w: 3.8, fontSize: 14, bold: true, color: "0f172a" });
    let yPos = 1.7;
    quadrantInsights.forEach(qi => {
      slideScatter.addText(qi, { x: 5.8, y: yPos, w: 3.8, fontSize: 11, color: "334155", bullet: true });
      yPos += 0.5;
    });
  }

  // 4. TOP KASUS DEFISIT (NATIVE BAR CHART & TABLE)
  if (topCases && topCases.length > 0) {
    const slideTop = pptx.addSlide();
    slideTop.addText(Top INA-CBG Defisit - KSM , titleProps);
    ...
    topCases.forEach((c, idx) => {
      tableRows.push([
        ...
        { text: formatRp(c.avgRs), options: { ...tableCellProps, align: "right" } },
        { text: formatRp(c.avgIna), options: { ...tableCellProps, align: "right" } },
        { text: formatRp(c.loss), options: { ...tableCellProps, align: "right", color: "e11d48", bold: true } }
      ]);
    });

    slideTop.addTable(tableRows, { x: 0.5, y: 1.5, w: 4.8, colW: [0.3, 0.9, 0.5, 1.0, 1.0, 1.1] });
  ...
  if (topUpPotentials && topUpPotentials.length > 0) {
    ...
    slideTopUp.addText(Potensi Top-Up (Severity Level) - KSM , titleProps);
  ...
  return pptx.writeFile({ fileName: Sosialisasi_KSM__.pptx });
};
"""
...
with open('src/utils/pptxExport.js', 'w', encoding='utf-8') as f:
    f.write(text)
print('Added generateSosialisasiPPTX!')
