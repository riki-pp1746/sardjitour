...
// 2. Add states and handlers (carefully)
const handlers = `
...
// 3. Add Top 10 Button
const t10Start = `<div className="mt-8 grid grid-cols-1 xl:grid-cols-2 gap-4 lg:col-span-3">`;
const t10Rep = `<div className="mt-8 lg:col-span-3">
...
if (content.includes(t10Start)) {
  content = content.replace(t10Start, t10Rep);
  
  // Close the wrapper
  const t10End = `<Top10Table title="Top 10 Tindakan Tidak Sesuai Kompetensi RS" data={data.top10?.procTidakSesuai} />
              </div>`;
  if (content.includes(t10End)) {
    // Add two closing divs to close grid and lg:col-span-3, and one for space-y-6 is closed by original `</div>\n          </div>\n        )}`
    content = content.replace(t10End, t10End + '\n            </div>');
  }
}
...
console.log('Safe inject complete!');
